...
export class DestructionPegSystem {
  ...
  wakeSleepingBodyFromKinematic(body, collider, contactPeg) {
    ...
    // The other collider is always KINEMATIC here (isKinematicCollider). A currently
    // script-animated (kinematic) body must not be detached by another kinematic carrier —
    // e.g. an animated group nested inside another animated group (concentric counter-
    // rotating rings, where an offset-pivot orbit gives one ring the linear velocity that
    // trips this path), or a bucket/flipper sweeping past it. It stays animation-owned
    // until a real impact (ball/blast via wakeBody) or active dynamic debris (via
    // maybeWakeSleepingCollisionBody, which is NOT gated on kinematic) hands it over.
    if (body.kinematic) return false;
